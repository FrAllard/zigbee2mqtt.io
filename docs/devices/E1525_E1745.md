...
#### Tips on pairing
The sensor is receiving commands from the coordinator only when it is sending its status. The sensor send its status once every 90 seconds. All the configuration take a long time to happen because of that, be patient. Here are some tips.
1. start from a dead device - remove batteries for at least 60 s or so
2. make sure that joining your zigbee network is allowed
3. insert the batteries with the device facing down, ideally on something soft such a sofa - the device should not spot you
4. press the connect button four times
5. wait for a notification to show up in the logs that the joining is in progress
6. point the device towards you so that it sees you and sends a message on the network
7. keep the device pointing at you and active until the end of the configuration
